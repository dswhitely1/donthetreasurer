...
export async function reassignLineItemCategories(
  _prevState: { error: string } | null,
  formData: FormData
) {
  const raw = {
  ...
  let parsedLineItems;
  try {
    parsedLineItems = lineItemsArraySchema.parse(
      JSON.parse(parsed.data.line_items)
    );
  } catch {
    return { error: "Invalid line items data." };
  }
  ...
  const { data: existing } = await supabase
    .from("transactions")
    .select("id, status, amount, account_id, transaction_line_items(id)")
  ...
  // This action is specifically for reconciled transactions
  if (existing.status !== "reconciled") {
    return { error: "Use the standard edit form for non-reconciled transactions." };
  }
  ...
  // Validate that line item amounts still equal the transaction total
  const lineItemSum = parsedLineItems.reduce((sum, li) => sum + li.amount, 0);
  if (Math.abs(lineItemSum - existing.amount) > 0.01) {
    return {
      error: `Line item amounts ($${lineItemSum.toFixed(2)}) must equal transaction total ($${existing.amount.toFixed(2)}).`,
    };
  }

  // Validate all category_ids
  const categoryIds = parsedLineItems.map((li) => li.category_id);
  const { data: validCategories } = await supabase
    .from("categories")
    .select("id, organization_id, is_active")
    .in("id", categoryIds);

  if (!validCategories || validCategories.length !== new Set(categoryIds).size) {
    return { error: "One or more categories not found." };
  }
  ...
  for (const cat of validCategories) {
    ...
    if (cat.organization_id !== parsed.data.organization_id) {
      return { error: "One or more categories do not belong to this organization." };
    }
  }
